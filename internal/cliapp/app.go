...
// Options supplies the process boundary for one command tree.
type Options struct {
	Version   string
	Reader    io.Reader
	Writer    io.Writer
...
// New constructs the SDD command tree. Run returns errors to its caller;
// --version stops early with a cli.ExitCoder whose exit code is zero.
func New(options Options) *cli.Command {
	...
	app := &cli.Command{
		...
		Flags: []cli.Flag{
			&cli.BoolFlag{
				...
				Action: func(_ context.Context, cmd *cli.Command, enabled bool) error {
					if !enabled {
						return nil
					}
		...
		Before: func(ctx context.Context, cmd *cli.Command) (context.Context, error) {
			...
			logger := slog.New(slogutils.NewCLIHandler(cmd.ErrWriter, &slogutils.CLIHandlerOptions{
				Level: level,
			}))
			...
			if cmd.Args().First() != "init" {
				...
				warnIfParticipantMissing(cmd.ErrWriter)
			}
			return ctx, nil
		},
	...
	setCommandStreams(app, options.Reader, options.Writer, options.ErrWriter)
	return app
}

func setCommandStreams(cmd *cli.Command, reader io.Reader, writer, errWriter io.Writer) {
	cmd.Reader, cmd.Writer, cmd.ErrWriter = reader, writer, errWriter
	for _, child := range cmd.Commands {
		setCommandStreams(child, reader, writer, errWriter)
	}
}
...
func infoCmd() *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			presenters.RenderInfo(cmd.Writer, result)
			return nil
		},
	}
}
...
func showCmd() *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			if err := freshenRepoCaches(ctx, cmd.ErrWriter, model.CrossRepoIDs(ids)); err != nil {
				return err
			}
			...
			if w, _, err := term.GetSize(cmd.Writer.(interface{ Fd() uintptr }).Fd()); err == nil {
				opts.Width = w
			}
			presenters.RenderShowStyled(cmd.Writer, result, opts)
			return nil
		},
	}
}
...
func newCmd() *cli.Command {
	return &cli.Command{
		...
		Action: withWriteGate(func(ctx context.Context, cmd *cli.Command) error {
			...
			cliAtts, err := parseAttachFlags(cmd.StringSlice("attach"), cmd.Reader)
			if err != nil {
				return err
			}
		...
		Action: withWriteGate(func(ctx context.Context, cmd *cli.Command) error {
			...
			rcmd := &command.RewriteEntryCmd{
				...
				OnRewritten: func(oldID, newID string, inbound []string) {
					if cmd.Bool("dry-run") {
						fmt.Fprintf(cmd.Writer, "would rewrite %s → %s\n", oldID, newID)
					} else {
						fmt.Fprintf(cmd.Writer, "%s → %s\n", oldID, newID)
					}
					if len(inbound) > 0 {
						fmt.Fprintf(cmd.Writer, "  inbound updates: %d\n", len(inbound))
...
func lintCmd() *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			if cmd.Bool("fix") {
				...
				fixCmd := &command.LintFixCmd{
					OnFixed: func(id string, fixes []string) {
						for _, f := range fixes {
							fmt.Fprintf(cmd.ErrWriter, "  fixed %s: %s\n", id, f)
						}
					},
				}
				reader, err := newReadFinder()
			...
			presenters.RenderLint(cmd.Writer, result)
		...
		Action: withWriteGate(func(ctx context.Context, cmd *cli.Command) error {
			...
			sumCmd := &command.SummarizeCmd{
				...
				OnSummarized: func(id, summary string) {
					fmt.Fprintf(cmd.ErrWriter, "  summarized %s\n", id)
				},
				OnSkipped: func(id string) {
					fmt.Fprintf(cmd.ErrWriter, "  skipped %s (already has a summary)\n", id)
				},
			}
...
func promptGraphDir(cmd *cli.Command, defaultValue string) (string, error) {
...
func promptParticipant(cmd *cli.Command, defaultValue string) (string, error) {
	return tui.RunTextPrompt(tui.TextPrompt{
		Reader:  cmd.Reader,
		Writer:  cmd.Writer,
...
// Empty input accepts the default; cancellation returns an error.
func promptLanguage(cmd *cli.Command, defaultValue string) (string, error) {
	return tui.RunTextPrompt(tui.TextPrompt{
		Reader:  cmd.Reader,
		Writer:  cmd.Writer,
...
func promptScope(cmd *cli.Command) (model.Scope, error) {
...
func promptAgents(cmd *cli.Command) ([]model.AgentTarget, error) {
...
//
// The root Before hook guards `sdd init` so the warning never
...
func warnIfParticipantMissing(errWriter io.Writer) {
	...
	if cfg == nil || strings.TrimSpace(cfg.Participant) == "" {
		fmt.Fprintln(errWriter, "sdd: no participant configured — run `sdd init` to set one")
	}
}
...
func promptOverwriteModified(cmd *cli.Command, absPath string) (bool, error) {
	return promptConfirmation(cmd, fmt.Sprintf("Overwrite user-edited %s?", absPath))
}

func promptConfirmation(cmd *cli.Command, prompt string) (bool, error) {
	return tui.RunConfirm(tui.ConfirmPrompt{Reader: cmd.Reader, Writer: cmd.Writer, Prompt: prompt})
}
...
func initCmd() *cli.Command {
	return &cli.Command{
		...
		Action: withWriteGate(func(ctx context.Context, cmd *cli.Command) error {
			...
			if cmd.Bool("bump") && model.IsDevVersion(cmd.Root().Version) {
			...
			icmd := &command.InitCmd{
				...
				OnMinimumVersionBumped: func(previous, current string) {
					if previous == "" {
						fmt.Fprintf(cmd.Writer, "  minimum_version: → %s\n", current)
					} else {
						fmt.Fprintf(cmd.Writer, "  minimum_version: %s → %s\n", previous, current)
					}
				},
				...
				OnCreated: func(sddDir, absGraphDir string) {
					...
					fmt.Fprintf(cmd.Writer, "  graph: %s\n", absGraphDir)
				},
				OnMigrated: func(count int) {
					fmt.Fprintf(cmd.ErrWriter, "  migrated %d file(s) from .sdd-tmp/\n", count)
				},
				OnGitignoreUpdated: func(path string) {
					fmt.Fprintf(cmd.ErrWriter, "  updated %s\n", path)
				},
				OnMetaWritten: func(path string) {
					fmt.Fprintf(cmd.Writer, "  meta: %s\n", path)
				},
				...
				OnSkillsInstalled: func(result command.SkillInstallResult) {
					...
					presenters.RenderInitSkills(cmd.Writer, result.InstallDir, result)
				},
				OnBridgeScaffolded: func(paths []string) {
					for _, p := range paths {
						fmt.Fprintf(cmd.Writer, "  bridge: %s\n", p)
					}
				},
				OnMCPRegistered: func(target model.AgentTarget, path string) {
					fmt.Fprintf(cmd.Writer, "  mcp (%s): %s\n", target, path)
				},
				OnAgentSkillsPruned: func(result command.AgentPruneResult) {
					presenters.RenderInitPrune(cmd.Writer, result)
				},
				OnSkillOrphansPruned: func(result command.AgentPruneResult) {
					presenters.RenderInitOrphans(cmd.Writer, result)
				},
				OnIndexMigrated: func(legacyDir, storeDir string, moved bool) {
					if moved {
						fmt.Fprintf(cmd.Writer, "  index migrated: %s → %s\n", legacyDir, storeDir)
						return
					}
					fmt.Fprintf(cmd.Writer, "  index store already exists at %s — the legacy copy at %s is unused and can be removed\n", storeDir, legacyDir)
				},
			}
			...
			notifyUnconnectedDependencies(cmd.ErrWriter, sddDir)
			return nil
		}),
	}
}
...
func notifyUnconnectedDependencies(errWriter io.Writer, sddDir string) {
	cfg, err := meta.ReadConfig(sddDir)
	...
	for _, dep := range gcfg.UnconnectedDependencies(cfg.Dependencies) {
		fmt.Fprintf(errWriter, "sdd: declared dependency %s is not connected on this machine — run `sdd repo add <clone-url>` (conventionally https://%s.git)\n", dep, dep)
	}
}
		...
		Action: withWriteGate(func(ctx context.Context, cmd *cli.Command) error {
			...
			startCmd := &command.StartWIPCmd{
				...
				OnStarted: func(markerID, markerPath string) {
					fmt.Fprintln(cmd.Writer, markerID)
					fmt.Fprintf(cmd.Writer, "  → %s\n", markerPath)
				},
				OnBranchCreated: func(branch string) {
					fmt.Fprintf(cmd.Writer, "  branch: %s (checked out)\n", branch)
				},
				OnExclusiveCollision: func(existing *model.WIPMarker) {
					fmt.Fprintf(cmd.ErrWriter, "warning: exclusive marker exists for %s by %s (%s)\n",
		...
		Action: withWriteGate(func(ctx context.Context, cmd *cli.Command) error {
			...
			doneCmd := &command.FinishWIPCmd{
				...
				OnBranchDeleted: func(branch string, forced bool) {
					if forced {
						fmt.Fprintf(cmd.Writer, "  force-deleted branch %s (unmerged)\n", branch)
					} else {
						fmt.Fprintf(cmd.Writer, "  deleted branch %s (merged)\n", branch)
					}
				},
				OnBranchPreserved: func(branch string) {
					fmt.Fprintf(cmd.ErrWriter, "  warning: branch %s has unmerged changes — marker removed but branch preserved\n", branch)
					fmt.Fprintln(cmd.ErrWriter, "  use --force to delete the unmerged branch, or merge it first")
				},
			}
...
func wipListCmd() *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			presenters.RenderWIPList(cmd.Writer, result)
			return nil
		},
	}
}
