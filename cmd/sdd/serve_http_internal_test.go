...
func TestServeHTTPTransportIsStateless(t *testing.T) {
	root := canonicalTempDir(t)
	if err := os.MkdirAll(filepath.Join(root, ".sdd", "graph"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".sdd", "config.yaml"), []byte("graph_dir: .sdd/graph\ndefault_branch: main\nrepo_id: example.test/http-smoke\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".sdd", "config.local.yaml"), []byte("participant: HTTP Smoke\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	cmd.Dir = root
	cmd.Env = append(os.Environ(), mainHelperArgsEnv+"=--graph-dir .sdd/graph serve --transport http --addr "+addr+" --auth-token http-test-token",
	...
	session, err := client.Connect(ctx, &mcp.StreamableClientTransport{
		...
		HTTPClient: &http.Client{Transport: localHTTPTestTransport{}},
	}, nil)
	if err != nil {
...
type localHTTPTestTransport struct{}

func (localHTTPTestTransport) RoundTrip(request *http.Request) (*http.Response, error) {
